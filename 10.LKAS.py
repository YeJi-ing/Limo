...
from time import *
'''
요약      : 슬라이딩 윈도우를 통해 차선 인식하고 차량이 중앙 유지하도록 제어하는 코드
...
class LKAS:
    ...
    def img_CB(self, data):
        ...
        if self.end_time - self.start_time >= 0.1: # 제어 명령을 얼마나 자주 보낼지 설정
            self.ctrl_pub.publish(ctrl_cmd_msg) # 차량 제어: 제어 명령을 ROS 토픽으로 Publish
        ...
        self.pub.publish(sliding_window_msg) # 이미지 Publish
